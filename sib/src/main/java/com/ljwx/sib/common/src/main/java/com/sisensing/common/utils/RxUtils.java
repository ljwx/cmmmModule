...
import com.sisensing.http.ExceptionHandle;
...
//                        .map(new HandleFuc<T>())  //这里可以取出BaseResponse中的Result
    ...
    private static class HandleFuc<T,U> implements Function<BaseResponse<T,U>, T> {
        @Override
        public T apply(BaseResponse<T,U> response) {
            if (!response.isOk())
