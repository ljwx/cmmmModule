package com.sisensing.common.view.vcedittext;
...
public class VerCodeEditText extends AppCompatEditText implements
    ...
    private int mCodeNum;//需要输入的位数
    private int mVerCodeWidth;//验证码宽
    ...
    private int mCursorBottomLineColor;//光标底部线的颜色
    ...
    private Paint mCursorBottomLinePaint;
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        mCodeNum = ta.getInteger(R.styleable.VerCodeEditText_verCodeNum, 4);
        mVerCodeWidth = (int) ta.getDimension(R.styleable.VerCodeEditText_verCodeWidth, 0);
        ...
        mCursorBottomLineColor = ta.getColor(R.styleable.VerCodeEditText_cursorBottomLineColor,getColor(R.color.color_00D5B8));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mVerCodeMargin = (widthResult - mVerCodeWidth * mCodeNum) / (mCodeNum - 1);
        //最终的高度
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mCodeNum; i++) {
            ...
            } else if (i == mCurrentPosition && hasFocus()){
                canvas.drawLine(start, lineY, end, lineY, mCursorBottomLinePaint);
            }else {
        ...
        //绘制光标
        if (!isCursorShowing && isCursorVisible() && mCurrentPosition < mCodeNum && hasFocus()) {
            canvas.save();
    ...
    public void afterTextChanged(Editable s) {
        mCurrentPosition = getText().length();
        postInvalidate();
        if (getText().length() == mCodeNum) {
            if (onCodeChangedListener != null) {
                onCodeChangedListener.onInputCompleted(getText());
            }
        } else if (getText().length() > mCodeNum) {
            getText().delete(mCodeNum, getText().length());
        }
    }
    ...
    @Override
    public void setCursorBottomLineColor(int cursorColor) {
        mCursorBottomLineColor = getColor(cursorColor);
        postInvalidate();
    }
