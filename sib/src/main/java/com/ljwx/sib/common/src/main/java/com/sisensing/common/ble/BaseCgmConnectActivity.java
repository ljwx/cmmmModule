 ...
 * Description:
 */
public abstract class BaseCgmConnectActivity<V extends ViewDataBinding, VM extends BaseViewModel> extends BaseActivity<V, VM> implements CgmConnectListener {
...
/*
        if (mBindNewDevice && !mQueriedDevice) {
            queryDevice(mLinkCode, mUserId);
        } else {
            dismissDialog();
            if (showToast){
                ToastUtils.showShort(getString(R.string.common_connect_fail));
            }
            connectFailCount++;
