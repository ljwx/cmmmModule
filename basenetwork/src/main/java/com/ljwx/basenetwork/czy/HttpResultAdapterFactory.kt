        ...
        override fun enqueue(callback: Callback<Any>): Unit =
            delegate.enqueue(object : Callback<Any> {
                override fun onResponse(call: Call<Any>, response: Response<Any>) {
                    try {
                        ...
                        val body = response.body()
                            ?: throw KotlinNullPointerException("response body was null")
                        callback.onResponse(
                            this@CallbackCall,
                            Response.success(HttpResult.Success(body), response.raw())
                        )
                    } catch (throwable: Throwable) {
                        globalErrorHandler?.invoke(throwable)
                        callback.onResponse(
                            this@CallbackCall,
                            Response.success(HttpResult.Failure<Any>(throwable), response.raw())
                        )
                    }
                }
                ...
                override fun onFailure(call: Call<Any>, throwable: Throwable) {
                    ...
                    callback.onResponse(
                        this@CallbackCall,
                        Response.success(HttpResult.Failure<Any>(throwable))
                    )
                }
            })
